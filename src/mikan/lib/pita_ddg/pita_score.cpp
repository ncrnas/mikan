        ...
        int pFlankDown)
{
    ...
    for (unsigned i = 0; i < length(mRNAPos); ++i)
    {
        if (!pSeedSites.mEffectiveSites[i] || !mDGDuplexScores.mEffectiveSites[i] || !mDGOpenScores.mEffectiveSites[i])
        {
            mEffectiveSites[i] = false;
            mDDGScores[i] = 0.0;
            continue;
...
template class PITAAlign<TRNATYPE>;
template class PITADGDuplexScores<TRNATYPE>;
template class PITADGOpenScores<TRNATYPE>;
template class PITADDGScores<TRNATYPE>;
template class PITATotalScores<TRNATYPE>;
