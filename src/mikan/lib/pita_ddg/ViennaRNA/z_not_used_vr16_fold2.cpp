...
float VR16Fold::fold(std::string &pString, std::string &pStructure)
{
    ...
    energy = finalize_energy_calc(strLen, pStructure, energy, mOpts, mBP);
...
//    int i, j, k, length, energy, new;
    ...
    while (pS > 0)
    {
        ...
        { /* trace back in fML array */
            ...
            if (pArrayML.mFML[mIdex[j] + i + 1] + mParams.mMLBase == fij)
            { /* 5' end is unpaired */
                mSector[++pS].i = i + 1;
                mSector[pS].j = j;
                mSector[pS].ml = ml;
            ...
            else if (mOpts.mDangles % 2 == 1)
            { /* normal dangles */
                tt = pPtype[mIdex[j] + i + 1];
                ci1j = pArrayC.mC[mIdex[j] + i + 1] + mParams.mDangle5[tt][pS1[i]] + mParams.mMLintern[tt] + mParams.mMLBase;

                tt = pPtype[mIdex[j - 1] + i];
                cij1 = pArrayC.mC[mIdex[j - 1] + i] + mParams.mDangle3[tt][pS1[j]] + mParams.mMLintern[tt] + mParams.mMLBase;

                tt = pPtype[mIdex[j - 1] + i + 1];
                ci1j1 = pArrayC.mC[mIdex[j - 1] + i + 1] + mParams.mDangle5[tt][pS1[i]] + mParams.mDangle3[tt][pS1[j]]
            ...
            if ((mOpts.mDangles == 3) && (k > j - 2 - TURN))
            { /* must be coax stack */
                ml = 2;
                for (k = i + 1 + TURN; k <= j - 2 - TURN; k++)
                {
        ...
        for (k = i + 2 + TURN; k < j - 2 - TURN; k++)
        {
            ...
            if (mOpts.mDangles % 2 == 1)
            { /* normal dangles */
                if (cij == (pArrayML.mFML[mIdex[k] + i + 2] + pArrayML.mFML[mIdex[j - 1] + k + 1] + mm + d3 + mParams.mMLBase))
                {
                    i1 = i + 2;
                    break;
                }
                if (cij == (pArrayML.mFML[mIdex[k] + i + 1] + pArrayML.mFML[mIdex[j - 2] + k + 1] + mm + d5 + mParams.mMLBase))
                {
                    j1 = j - 2;
                    break;
                }
            ...
            if (mOpts.mDangles == 3)
            {
                ...
                if (type_2)
                {
                    en = pArrayC.mC[mIdex[k] + i + 1] + mParams.mStack[type_1][type_2] + pArrayML.mFML[mIdex[j - 1] + k + 1];
                ...
                if (type_2)
                {
                    en = pArrayC.mC[mIdex[j - 1] + k + 1] + mParams.mStack[type_1][type_2] + pArrayML.mFML[mIdex[k] + i + 1];
                    if (cij == en + 2 * mParams.mMLintern[1] + mParams.mMLclosing)
                    {
                        mSector[pS + 2].ml = 2;
                        break;
